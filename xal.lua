...
local Drawing = loadstring(game:HttpGet("https://pastebin.com/raw/j7w9CVM4"))();

local Library = {};
do
	Library = {
		Open = true;
		Accent = Color3.fromRGB(270, 0, 0);
		Pages = {};
		Sections = {};
		Flags = {};
		UnNamedFlags = 0;
	...
	do
		...
		function Library:New(Properties)
			...
			Logo.Name = "Logo"
			Logo.Image = "http://www.roblox.com/asset/?id=73996534932277"
		...
		function Library:Watermark(Properties)
			local Watermark = {
				Name = (Properties.Name or Properties.name or "xal.ware | daddy xaloh made");
				AnimateText = nil;
			}
			--
			local Outline = Instance.new("Frame")
			...
			task.spawn(function()
				while task.wait() do
					for i = 1, #"xal.ware" do
						Watermark.AnimateText = string.sub("xal.ware", 1, i) .. "";
						Title.Text = Watermark.AnimateText .. " " .. Watermark.Name;
						task.wait(0.4);
					end;

					for i = #"xal.ware" - 1, 1, -1 do
						Watermark.AnimateText = string.sub("xal.ware", 1, i) .. "";
						Title.Text = Watermark.AnimateText .. " " .. Watermark.Name;
						task.wait(0.4);
					end;
...
--// window locals
local default_color = Color3.fromRGB(270, 0, 0);
...
do
	...
	do
		...
		instances["local_text"] = utility.instance_new("TextLabel", { 
			Name = "xal.ware",
			Parent = screen_gui_2,
			Text = '<font color="rgb(207, 227, 0)">xal</font><font color="rgb(255, 255, 255)">.ware</font>',
		...
		do
			...
			utility.new_connection(run_service.Heartbeat, function()
				...
				if (enabled) then
					...
					local text = custom_text_enabled and '<font color="' .. text_color .. '">' .. features.get_text(custom_text) .. '</font>' or '<font color="' .. text_color .. '">xal.</font><font color="rgb(255, 255, 255)">ware</font>';
...
do
	...
	ui.tabs["legit"] = window:Page({Name = "Lock", Icon = "http://www.roblox.com/asset/?id=6023426921"});
	...
	do --// credits to finobe wtv im way too lazy
		...
		if not isfolder("xalware/configs") then 
		    makefolder("xalware/configs");
		end;

		local function update_config_list()
		    local list = {};
		
		    for idx, file in listfiles("xalware/configs") do
		        local file_name = file:gsub("xalware/configs\\", ""):gsub(".cfg", ""):gsub("xalware/configs/", "");
		...
		cfgs:Button({Name = "Create", Callback = function()
		    ...
		    if config_name == "" or isfile("xalware/configs/" .. config_name .. ".cfg") then
		        return;
		    end;
		    writefile("xalware/configs/" .. config_name .. ".cfg", Library:GetConfig());
		    update_config_list();
		end});
		...
		cfgs:Button({Name = "Save", Callback = function()
		    ...
		    if selected_config then
		        writefile("xalware/configs/" .. selected_config .. ".cfg", Library:GetConfig());
		    end;
		end});
		...
		cfgs:Button({Name = "Load", Callback = function()
		    local selected_config = flags.setting_configuration_list;
		    if selected_config then
		        Library:LoadConfig(readfile("xalware/configs/" .. selected_config .. ".cfg"));
		    end;
		end});
		...
		cfgs:Button({Name = "Delete", Callback = function()
		    local selected_config = flags.setting_configuration_list;
		    if selected_config then
		        delfile("starxalwarehook/configs/" .. selected_config .. ".cfg");
		    end;
		    update_config_list();
		end});
